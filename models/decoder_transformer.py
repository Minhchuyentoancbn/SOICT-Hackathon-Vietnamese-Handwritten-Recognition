...
class AttentionRecognitionHead(nn.Module):
  ...
  def forward(self, x):
    ...
    batch_targets = torch.zeros(batch_size, max_lens).to(x.device)
  ...
  def sample(self, x):
    ...
    outputs = torch.zeros(batch_size, self.max_len_labels).to(x.device)
...
class DecoderUnit(nn.Module):
  ...
  def forward(self, x, sPrev, yPrev):
    ...
    self.target_mask = self._generate_square_subsequent_mask(yPrev.size()[1]).to(x.device)
    self.src_mask  = self._generate_square_subsequent_mask(x.size()[1]).to(x.device)
    # print("src_mask: ", self.src_mask.size())
    # print(" ",self.target_mask.size()) 
    self.memory_mask = self._generate_square_subsequent_mask(x.size()[1]).to(x.device)
