...
class ResNet_ASTER_FeatureExtractor(nn.Module):

    def __init__(self, input_channel, output_channel=512):
        super(ResNet_ASTER_FeatureExtractor, self).__init__()
        self.ConvNet = ResNet_ASTER(input_channel, output_channel)
...
class ResNet_ASTER(nn.Module):
    ...
    def __init__(self, input_channel, output_channel):
        ...
        self.inplanes = output_channel / 16

        self.layer0 = nn.Sequential(
            nn.Conv2d(self.input_channel, 32, kernel_size=(3, 3), stride=1, padding=1, bias=False),
            nn.BatchNorm2d(32),
            nn.ReLU(inplace=True)
        )

        self.layer1 = self._make_layer(output_channel / 16,  3, [2, 2]) # [16, 50]
        self.layer2 = self._make_layer(output_channel / 8,  4, [2, 2]) # [8, 25]
        self.layer3 = self._make_layer(output_channel / 4, 6, [2, 1]) # [4, 25]
        self.layer4 = self._make_layer(output_channel / 2, 6, [2, 1]) # [2, 25]
        self.layer5 = self._make_layer(output_channel, 3, [2, 1]) # [1, 25]
